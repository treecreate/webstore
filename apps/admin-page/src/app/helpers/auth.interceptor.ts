  ...
  intercept(
    ...
    if (authUser != null) {
      ...
      if (req.url.includes('/auth/refresh')) {
        authReq = req.clone({
          headers: req.headers.set(TOKEN_HEADER_KEY, 'Bearer ' + authUser.refreshToken),
        });
        return next.handle(authReq);
      }
      // check that the token is not expired, and perform a refresh if it is
      if (this.authService.isJwtExpired(authUser.accessToken)) {
        ...
        authReq = req.clone({
          headers: req.headers.set(TOKEN_HEADER_KEY, 'Bearer ' + authUser.accessToken),
        });
        return next.handle(authReq);
      }
    } else {
