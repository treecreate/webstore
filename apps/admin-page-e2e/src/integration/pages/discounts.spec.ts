...
describe('discountsPage', () => {
  beforeEach(() => {
    ...
    cy.intercept(
      { method: 'GET', url: 'http://localhost:5050/discounts' },
      {
        body: mockDiscounts,
        statusCode: 200,
      }
...
describe('create discount dialog', () => {
  beforeEach(() => {
    ...
    cy.intercept('GET', 'http://localhost:5050/discounts', {
      body: mockDiscounts,
      statusCode: 200,
    }).as('fetchDiscounts');
  ...
  it('should create a discount', () => {
    ...
    cy.get('[data-cy=discount-expires-at-input]').type('2022-10-10');
    cy.get('[data-cy=discount-type-select]').select('PERCENT').should('have.value', 'PERCENT');
    cy.get('[data-cy=discount-amount-input]').type('25');
