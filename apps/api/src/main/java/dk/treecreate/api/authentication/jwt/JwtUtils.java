...
public class JwtUtils {
  ...
  /**
   * Check if the given token is valid and whitelisted or not Prints to console why the token is
   * invalid
   *
   ...
   *
   * @param token a JWT.
   * @return amount of seconds left until the token expires; <code>0</code> if the token is already
   *     expired.
  ...
  /**
   * Adds a token pair to the whitelist, where the key is the <code>refreshToken</code> and the
   * value is the <code>authToken</code>.
   *
   * <p>The entry's TTL is equal to the <code>refreshToken</code>'s TTL.
   *
   * @param authToken
   * @param refreshToken
   */
  ...
  /**
   * Removes a token pair from the whitelist based on the provided token. Provided token can be
   * wither an <code>authToken</code>(value), or <code>refreshToken</code>(key).
   *
   * @param token
   */
