...
public class ShipmentObject {
   ...
   * @param label Label object specifying where to send the shipping label to
   *     <p>For now it's hardcoded to Teodor's email
   */
  public ShipmentObject(
      boolean test_mode,
    ...
    this.product_code =
        isHomeDelivery
            ? "GLSDK_HD"
            : "GLSDK_SD"; // GLS home delivery or Store Delivery (pickup point)
    // TODO - Implement required phone number for home delivery requests
    this.service_codes =
        isHomeDelivery
            ? "EMAIL_NT,SMS_NT"
            : "EMAIL_NT"; // home delivery requires both sms and email notification
    this.sender = sender;
  ...
  public String toString() {
    return "{"
        ...
        + getAutomatic_select_service_point()
        + "'"
        + ", sender='"
        + getSender()
        + "'"
