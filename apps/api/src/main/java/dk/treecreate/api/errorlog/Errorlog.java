...
public class Errorlog
{
    ...
    @GenericGenerator(name = "UUID", strategy = "org.hibernate.id.UUIDGenerator", parameters = {
        @org.hibernate.annotations.Parameter(name = "uuid_gen_strategy_class",
            value = "org.hibernate.id.uuid.CustomVersionOneStrategy")}) @Type(type = "uuid-char")
    @Column(name = "errorlog_id", updatable = false, nullable = false)
    @ApiModelProperty(notes = "UUID of the errorlog entity",
        example = "c0a80121-7ac0-190b-817a-c08ab0a12345")
    private UUID errorlogId;

    @NotBlank @Size(max = 80)
    @ApiModelProperty(name = "The name of the errorlog", example = "webstore.login.login-failed",
        required = true)
    private String name;

    // The userId is not a relation because 1. JPA sucks, and 2. Doesn't need to be
    @Type(type = "uuid-char") @Column(name = "user_id", nullable = false) @ApiModelProperty(
        notes = "Id of the errorlogs's user. Doesn't correspond to database user if it was made when not logged in",
        example = "c0a80121-7ac0-190b-817a-c08ab0a12345")
    private UUID userId;
    ...
    @NotNull
    @ApiModelProperty(name = "Whether the error has been resolved by the developers",
    ...
    @Override public boolean equals(Object o)
    {
        ...
        return errorlogId.equals(errorlog.errorlogId) && name.equals(errorlog.name) &&
            Objects.equals(userId, errorlog.userId) && Objects.equals(browser, errorlog.browser) &&
            Objects.equals(url, errorlog.url) && Objects.equals(production, errorlog.production) &&
    ...
    {
        return Objects.hash(errorlogId, name, userId, browser, url, production, createdAt,
            updatedAt);
    }
}
