...
public class DesignController {
  ...
  public List<Design> getAllOfSpecificUser(
      @ApiParam(name = "userID", example = "c0a80121-7ac0-190b-817a-c08ab0a12345") @PathVariable
          UUID userId) {
  ...
  @ApiResponses(
      value = {@ApiResponse(code = 200, message = "Design entity", response = Design.class)})
  @PreAuthorize("hasRole('DEVELOPER') or hasRole('ADMIN')")
  public Design getDesign(
      @ApiParam(name = "designId", example = "c0a80121-7ac0-190b-817a-c08ab0a12345") @PathVariable
          UUID designId) {
    ...
    if (design.getUser().getUserId() != currentUser.getUserId()) {
      ...
      if (!isAdmin) {
        throw new ResponseStatusException(
            HttpStatus.FORBIDDEN, "The design belongs to another user");
      }
  ...
  @ApiResponses(
      value = {
        @ApiResponse(code = 200, message = "Design information", response = Design.class),
      })
  @PreAuthorize("hasRole('USER') or hasRole('DEVELOPER') or hasRole('ADMIN')")
  ...
  @ApiResponses(
      value = {
        @ApiResponse(code = 200, message = "Design entity.", response = Design.class),
        @ApiResponse(code = 404, message = "Design not found.")
      })
  @PreAuthorize("hasRole('USER') or hasRole('DEVELOPER') or hasRole('ADMIN')")
  public Design update(@RequestBody() @Valid UpdateDesignRequest updateDesignRequest) {
    var userDetails = authUserService.getCurrentlyAuthenticatedUser();
    User currentUser =
        userRepository
            .findByEmail(userDetails.getUsername())
    ...
    Design design =
        designRepository
            .findByDesignId(updateDesignRequest.getDesignId())
            .orElseThrow(() -> new ResourceNotFoundException("Design not found"));
    if (design.getUser().getUserId() != currentUser.getUserId()) {
      throw new ResponseStatusException(HttpStatus.FORBIDDEN, "The design belongs to another user");
    }

    if (!design.isMutable()) {
      throw new ResponseStatusException(HttpStatus.FORBIDDEN, "The design can't be updated");
    }
