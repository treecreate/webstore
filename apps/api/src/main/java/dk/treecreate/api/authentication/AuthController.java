...
public class AuthController {
  ...
  @ApiResponses(
      value = {
        @ApiResponse(
            code = 200,
            message = "User information including the access token",
  ...
  public ResponseEntity<JwtResponse> authenticateUser(
    ...
    return ResponseEntity.ok(
        authUserService.authenticateUser(
            loginRequest.getEmail(), loginRequest.getPassword(), eventLogUserId));
  }
  ...
  @ApiResponses(
      value = {
        @ApiResponse(
            code = 200,
            message = "Information about the newly created user",
            response = JwtResponse.class),
        @ApiResponse(
            code = 401,
  ...
  public ResponseEntity<JwtResponse> registerUser(
    ...
    User user =
        new User(
            signUpRequest.getEmail(), // for simplicity, the username is the email
    ...
    try {
      ...
      throw new ResponseStatusException(
          HttpStatus.INTERNAL_SERVER_ERROR, "Failed to send the email. Try again later");
    }

    return ResponseEntity.ok(
        authUserService.authenticateUser(
            signUpRequest.getEmail(), signUpRequest.getPassword(), eventLogUserId));
  }
  ...
  /**
   * Logs out the user by taking the token from the <code>Authorization</code> header, extracting
   * the username from its body, and invalidating all of the tokens that belong to that user.
   *
   * @param request an HTTP request.
  ...
  public void logout(HttpServletRequest request) {
    try {
      ...
      throw new ResponseStatusException(
          HttpStatus.INTERNAL_SERVER_ERROR, "Failed to logout. Try again later.");
    }
  }
}
