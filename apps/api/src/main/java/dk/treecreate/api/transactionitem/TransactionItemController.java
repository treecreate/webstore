...
public class TransactionItemController {
  ...
  @ApiResponses(
      value = {
        @ApiResponse(
            code = 200,
            message = "Transaction item information",
  ...
  public TransactionItem getTransactionItem(
    ...
    return transactionItemRepository
        .findByTransactionItemId(transactionItemId)
  ...
  public TransactionItem getCurrentTransactionItem(
    ...
    if (transactionItem.getDesign().getUser().getUserId() != user.getUserId()) {
      throw new ResponseStatusException(
          HttpStatus.FORBIDDEN, "You lack clearance to view this resource");
    }
  ...
  public TransactionItem create(
    ...
    Design design =
        designRepository
            .findByDesignId(createTransactionItemRequest.getDesignId())
            .orElseThrow(() -> new ResourceNotFoundException("Design not found"));

    if (design.getUser().getUserId() != user.getUserId()) {
      throw new ResponseStatusException(
          HttpStatus.FORBIDDEN, "You lack clearance to use this design resource");
    }
    ...
    switch (design.getDesignType()) {
      ...
      case PET_SIGN:
        if (createTransactionItemRequest.getDimension().equals(DesignDimension.ONE_SIZE)) {
          throw new ResponseStatusException(
              HttpStatus.BAD_REQUEST,
              "Design of type 'PET_SIGN' can't be saved with dimension 'ONE_SIZE'");
        }
        break;
      default:
        break;
  ...
  @PostMapping("me/bulk")
  @Operation(summary = "Create multiple transaction items and their designs with a single request")
  @ApiResponses(
      value = {
        @ApiResponse(
            code = 200,
            message = "List of transaction items and their designs",
            response = GetTransactionItemsResponse.class)
      })
  @PreAuthorize("hasRole('USER') or hasRole('DEVELOPER') or hasRole('ADMIN')")
  @Transactional()
  ...
  public TransactionItem update(
    ...
    if (transactionItem.getDesign().getUser().getUserId() != user.getUserId()) {
      throw new ResponseStatusException(
          HttpStatus.FORBIDDEN, "You lack clearance to modify this resource");
    }
  ...
  public void delete(
    ...
    if (transactionItem.getDesign().getUser().getUserId() != user.getUserId()) {
      throw new ResponseStatusException(
          HttpStatus.FORBIDDEN, "You lack clearance to delete this resource");
    }
