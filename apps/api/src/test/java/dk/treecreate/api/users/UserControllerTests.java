...
class UserControllerTests {
  ...
  class AuthenticationTests {
    ...
    void getUserReturnsUnauthorizedOnInvalidCredentials() throws Exception {
      ...
      mvc.perform(get("/users/" + uuid)).andExpect(status().isUnauthorized());
    }
    ...
    void updateUserReturnsUnauthorizedOnInvalidCredentials() throws Exception {
      UUID uuid = new UUID(0, 0);
      mvc.perform(put("/users/" + uuid)).andExpect(status().isUnauthorized());
    }
    ...
    void updateCurrentUserReturnsUnauthorizedOnInvalidCredentials() throws Exception {
      mvc.perform(put("/users")).andExpect(status().isUnauthorized());
    }
    ...
    void deleteUserReturnsUnauthorizedOnInvalidCredentials() throws Exception {
      UUID uuid = new UUID(0, 0);
      mvc.perform(delete("/users/" + uuid)).andExpect(status().isUnauthorized());
    }
  }
  ...
  class GetUserTests {
    ...
    void getUserReturnsForbiddenToRoleUser() throws Exception {
      mvc.perform(get("/users/" + new UUID(0, 0))).andExpect(status().isForbidden());
    }
  ...
  class UpdateUserTests {
    ...
    void putUserReturnsForbiddenToRoleUser() throws Exception {
      mvc.perform(put("/users/" + new UUID(0, 0))).andExpect(status().isForbidden());
    }

    @Test
    @DisplayName("PUT /users endpoint returns updated version of the currently authenticated user")
    @WithMockUser(username = "user@hotdeals.dev", password = "testPassword")
    ...
    @WithMockUser(
        ...
        password = "testPassword",
        roles = {"DEVELOPER"})
    void updateUserWithTooLongEntryReturnsBadRequest() throws Exception {
      UUID userId = new UUID(0, 0);
      UpdateUserRequest updateUserRequest = new UpdateUserRequest();
  ...
  class DeleteUserTests {
    @Test
    @DisplayName("DELETE /users/:userId endpoint returns 403: Forbidden when called by ROLE_USER")
    @WithMockUser(username = "user@hotdeals.dev", password = "testPassword")
    void deleteUserReturnsForbiddenToRoleUser() throws Exception {
      mvc.perform(delete("/users/" + new UUID(0, 0))).andExpect(status().isForbidden());
    }
    ...
    void updateUserWithTooLongEntryReturnsBadRequest() throws Exception {
      ...
      mvc.perform(delete("/users/" + userId)).andExpect(status().isNoContent());
    }
  }
}
