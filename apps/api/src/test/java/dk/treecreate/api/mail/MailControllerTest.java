...
@WebMvcTest(value = MailController.class, excludeAutoConfiguration = SecurityAutoConfiguration.class)
class MailControllerTest
{
  ...
  void signupMessageBadRequest() throws Exception
  {
    ...
    mvc.perform(post("/mail/signup")
      ...
      .andExpect(result -> assertEquals("Provided email is not a valid email", result.getResponse().getErrorMessage()));
  }
  ...
  void signupMessageException() throws Exception
  {
    ...
    mvc.perform(post("/mail/signup")
      ...
      .contentType(MediaType.APPLICATION_JSON))
      // the message comes from a ResponseStatusException so has to be handled differently from normal content
      .andExpect(result -> assertEquals("Failed to send an email", result.getResponse().getErrorMessage()));
  }
