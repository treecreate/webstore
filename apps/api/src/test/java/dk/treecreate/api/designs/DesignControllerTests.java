...
public class DesignControllerTests {
  ...
  class AuthenticationTests {
    ...
    void getDesignsReturnsUnauthorizedOnInvalidCredentials() throws Exception {
      mvc.perform(get("/designs")).andExpect(status().isUnauthorized());
    }
    ...
    void getDesignsReturnsForbiddenOnMIssingRole() throws Exception {
      mvc.perform(get("/designs")).andExpect(status().isForbidden());
    }

    @Test
    @DisplayName("GET /designs/:userId endpoint returns 401 when user credentials are invalid")
    void getDesignsOfUserReturnsUnauthorizedOnInvalidCredentials() throws Exception {
      UUID uuid = new UUID(0, 0);
      mvc.perform(get("/designs/" + uuid)).andExpect(status().isUnauthorized());
    }
    ...
    void getDesignsOfUserReturnsForbiddenOnMissingRole() throws Exception {
      UUID uuid = new UUID(0, 0);
      mvc.perform(get("/designs/" + uuid)).andExpect(status().isForbidden());
    }

    @Test
    @DisplayName("POST /designs/:designId endpoint returns 401 when user credentials are invalid")
    void getDesignReturnsUnauthorizedOnInvalidCredentials() throws Exception {
      UUID uuid = new UUID(0, 0);
      mvc.perform(post("/designs/" + uuid)).andExpect(status().isUnauthorized());
    }
    ...
    void getDesignReturnsForbiddenOnMissingRole() throws Exception {
      UUID uuid = new UUID(0, 0);
      mvc.perform(post("/designs/" + uuid)).andExpect(status().isForbidden());
    }
    ...
    void getDesignsOfCurrentUserReturnsUnauthorizedOnInvalidCredentials() throws Exception {
      mvc.perform(get("/designs/me")).andExpect(status().isUnauthorized());
    }

    @Test
    @DisplayName("GET /designs/me/:designId endpoint returns 401 when user credentials are invalid")
    void getSpecificDesignOfCurrentUserReturnsUnauthorizedOnInvalidCredentials() throws Exception {
      UUID uuid = new UUID(0, 0);
      mvc.perform(get("/designs/me/" + uuid)).andExpect(status().isUnauthorized());
    }
    ...
    @Test
    @DisplayName("PUT /designs/:designId endpoint returns 401 when user credentials are invalid")
    void updateReturnsUnauthorizedOnInvalidCredentials() throws Exception {
      UUID uuid = new UUID(0, 0);
      mvc.perform(put("/designs/" + uuid)).andExpect(status().isUnauthorized());
    }

    @Test
    @DisplayName("DELETE /designs/:designId endpoint returns 401 when user credentials are invalid")
    void deleteReturnsUnauthorizedOnInvalidCredentials() throws Exception {
      UUID uuid = new UUID(0, 0);
      mvc.perform(delete("/designs/" + uuid)).andExpect(status().isUnauthorized());
    }
  }
  ...
  class GetDesignsOfCurrentUserTests {
    ...
    void getDesignsOfCurrentUserReturnsDesigns() throws Exception {
      ...
      Mockito.when(authUserService.getCurrentlyAuthenticatedUser()).thenReturn(user);
  ...
  class GetDesignsOfUserTests {
    ...
    @Test
    @DisplayName("GET /designs/:userId endpoint returns 403: Forbidden when called by ROLE_USER")
    @WithMockUser(username = "user@hotdeals.dev", password = "testPassword")
    void getDesignsOfUserReturnsForbiddenToRoleUser() throws Exception {
      mvc.perform(get("/designs/" + new UUID(0, 0))).andExpect(status().isForbidden());
    }
  ...
  class GetDesignTests {
    ...
    @Test
    @DisplayName("POST /designs/:designId endpoint returns 403: Forbidden when called by ROLE_USER")
    @WithMockUser(username = "user@hotdeals.dev", password = "testPassword")
  ...
  class GetDesignOfCurrentUserTests {
    ...
    @WithMockUser(username = "user@hotdeals.dev", password = "testPassword")
    void getDesignOfCurrentUserReturnsForbiddenWhenAccessingDesignOfAnotherUser() throws Exception {
      UUID uuid = new UUID(0, 0);
      User user = new User();
      ...
      mvc.perform(get("/designs/me/" + uuid)).andExpect(status().isForbidden());
    }
  ...
  class CreateDesignTests {
    ...
    void createDesignReturnsDesign() throws Exception {
      ...
      Mockito.when(userRepository.findByEmail(user.getEmail())).thenReturn(Optional.of(user));
      Mockito.when(authUserService.getCurrentlyAuthenticatedUser()).thenReturn(user);
      Mockito.when(designRepository.save(savedDesign)).thenReturn(savedDesign);
  ...
  class UpdateDesignTests {
    ...
    void updateDesignReturnsDesign() throws Exception {
      ...
      Mockito.when(designRepository.findByDesignId(design.getDesignId()))
          .thenReturn(Optional.of(design));
      Mockito.when(designRepository.save(design)).thenReturn(updatedDesign);
  ...
  class DeleteDesignTests {
    ...
    void deleteDesignReturnsForbiddenWhenDeletingDesignOfAnotherUser() throws Exception {
      ...
      mvc.perform(delete("/designs/" + uuid)).andExpect(status().isForbidden());
    }
    ...
    void deleteDesignReturnsNoContent() throws Exception {
      ...
      mvc.perform(delete("/designs/" + uuid)).andExpect(status().isNoContent());
    }
  }
}
