...
class AuthTestControllerTests {
  ...
  void allReturnsCorrectDataWithoutAuth() throws Exception {
    mvc.perform(get("/auth/test/all").contentType(MediaType.APPLICATION_JSON))
  ...
  void userReturnsUnauthorizedToUnauthorizedUsers() throws Exception {
    mvc.perform(get("/auth/test/user").contentType(MediaType.APPLICATION_JSON))
  ...
  void developerReturnsUnauthorizedToUnauthenticatedUsers() throws Exception {
    mvc.perform(get("/auth/test/developer").contentType(MediaType.APPLICATION_JSON))
        .andExpect(status().isUnauthorized());
  }
  ...
  // Because of how MockMvc and security work, failed requests return 200 with an empty body.
  // ¯\_(ツ)_/¯
  @Test
  ...
  void adminReturnsUnauthorizedToUnauthenticatedUsers() throws Exception {
    mvc.perform(get("/auth/test/admin").contentType(MediaType.APPLICATION_JSON))
        .andExpect(status().isUnauthorized());
  }
